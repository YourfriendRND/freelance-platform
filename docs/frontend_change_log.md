...
## Epic 2: FE-02 - Страница просмотра задачи

Добавлена публичная страница задачи: переход со списка, загрузка по id.

### Страница и навигация

- Добавлен маршрут `/tasks/:id`
- Карточка в списке ведет на страницу задачи
- Со страницы можно вернуться к списку по ссылке «Назад к задачам»
- Оболочка та же, что у списка: дашборд, меню «Задачи», шапка
- Страница доступна и гостю, и авторизованному пользователю

### API и состояние

- Добавлен запрос одной задачи по id
- Выбранная задача хранится отдельно от списка, чтобы детали и список не мешали друг другу
- Если открыть `/tasks/:id` напрямую, подгружается справочник категорий; если категории уже есть в сторе - повторно не запрашиваются
- Возврат со страницы задачи к списку не дергает список заново (пока без учета пагинации)
- Название категории берется из справочника; если категории нет - «Без категории»

### Редиректы по сессии

- Корень `/` после проверки сессии: гость => `/welcome`, есть сессия => `/tasks`
- Гостю доступны маршруты `/tasks` и `/tasks/:id`
- После успешного входа переход на `/tasks`
- Выход со списка и со страницы задачи ведёт на `/welcome`

### Тесты

- Добавлены unit-тесты страницы задачи, блока описания, боковой колонки, блока заказчика и store
- Добавлены тесты редиректа с `/`, проверки сессии, входа на `/tasks` и выхода на `/welcome`
