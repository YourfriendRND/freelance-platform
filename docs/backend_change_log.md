...
## Epic 2 BE-06 – Практика SQL и агрегация данных

В репозиториях задач и категорий задач появились выборки для аналитики заказчика. Все запросы ограничены задачами текущего заказчика.

### Задачи

– Количество задач, сгруппированных по статусу
– Минимальный, максимальный и средний бюджет по всем задачам заказчика. Если задач нет, агрегаты отдаются как ноль

### Категории

– Количество задач по категориям: соединение таблицы категорий с задачами, в выборку попадают только категории, в которых у заказчика есть задачи
– Минимальный и максимальный бюджет и сумма верхней границы бюджета по каждой такой категории
– Категории, в которых число задач заказчика не меньше заданного порога: порог проверяется после группировки, а не в обычном фильтре строк

## Epic 2 BE-05 - Модуль откликов на задачи

Добавлен домен откликов исполнителей на задачи: таблица, модуль и REST API по сессии.

### API

- `GET /api/task-applications` - список откликов текущего пользователя: исполнитель видит свои, заказчик - отклики на свои задачи
- Для заказчика доступен фильтр `?taskId=` - отклики по одной своей задаче; чужая или несуществующая задача => 404; параметр недоступен исполнителю => 400
- `GET /api/task-applications/:id` - детали отклика; смотреть может автор отклика или владелец задачи, иначе 403
- `POST /api/task-applications` - подача отклика (тело: задача, сообщение, опционально цена)
- `PATCH /api/task-applications/:id` - изменение отклика по правилам роли
- Все маршруты требуют сессию; без cookie => 401

### Правила

- Откликнуться может только исполнитель и только на открытую задачу; на свою задачу откликаться нельзя
- Повторный отклик того же исполнителя на ту же задачу => 409
- Исполнитель может менять сообщение и цену, пока статус `pending`; другой статус => 403
- Заказчик может выставить только `accept` или `decline` по отклику на свою задачу

### Тесты

- Unit-тесты сервиса, репозитория и DTO
- E2E: сценарий create => list => get => update; негативы 401/403/404/409


## Epic 2 BE-04 - Фильтрация, сортировка и пагинация задач

Публичный список задач расширен query-параметрами, серверной фильтрацией, сортировкой и постраничной выдачей.

### API

- `GET /api/tasks` принимает опциональные фильтры: категория, статус, диапазон бюджета, сортировка, номер страницы и размер страницы
- Ответ - объект с полями задач на странице, общим количеством по фильтрам, текущей страницей и лимитом (раньше возвращался плоский массив)
- По умолчанию страница 1 и лимит 20, максимальный лимит 250

### Фильтры

- Категория - по идентификатору; валидный UUID без задач даёт пустой список, невалидный UUID - ошибка валидации
- Статус - только `open` или `closed`; `draft` в фильтре запрещен
- Без параметра статуса в выдаче только открытые и закрытые задачи, черновики скрыты
- Бюджет - нижняя и верхняя граница; задача попадает в выборку, если ее диапазон полностью входит в заданный (минимум не ниже, максимум не выше);

### Сортировка

- Варианты: сначала новые, сначала старые, бюджет по возрастанию (по нижней границе), бюджет по убыванию (по верхней границе)
- По умолчанию - сначала новые

### Пагинация

- `LIMIT` и `OFFSET` с параметрами; отдельный подсчёт общего числа записей с теми же условиями фильтрации, что и у выборки

### Тесты

- Unit-тесты валидации query DTO
- Unit-тесты сервиса - проброс параметров в репозиторий
- Unit-тесты репозитория на mock БД - условия `WHERE`, `ORDER BY`, `LIMIT`/`OFFSET`, `COUNT`
- E2E - форма ответа, фильтры, сортировка, страницы, границы лимита, пустая выдача по категории без задач; сценарий CRUD учитывает, что черновик не попадает в публичный список
